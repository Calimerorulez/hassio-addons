...
if ! getent hosts "${HOST}" >/dev/null 2>&1; then
    ...
    bashio::log.warning "Nginx will still start; restart the app after DNS becomes available."
fi

bashio::log.info "Configuring proxy target: ${SERVER}"

sed \
    -e "s|%%NODERED_SERVER%%|${SERVER}|g" \
    "${TEMPLATE}" > "${CONFIG}"
...
bashio::log.info "Starting Node-RED Proxy on the internal Ingress port 8099."
